...
const GameInfoOverlay = ({ info }: Props) => {
    ...
    for (let i = 0; i < livesRemainingState; i++) {
        lives.push(<Image src={'/balloon.png'} alt={''} width={30} height={30}/>)
    }
    return (
        <div className={`absolute z-30 h-screen w-screen flex items-center justify-center ${isColliding && 'border-[10px] border-red-600'}`}>
